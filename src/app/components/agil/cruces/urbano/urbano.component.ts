    ...
    this.actividadesForm.get('act').valueChanges.subscribe(values => {
      ...
      ctrl.controls.forEach((x) => {
        ...
        x.patchValue({
          ...
          totalDias: totaldias,
          promedio: isFinite(promedio) ? promedio.toLocaleString(undefined, { maximumFractionDigits: 0 }) : 0,
          totalVentas: isFinite(totalbrm) ? totalbrm.toLocaleString() : 0,
          totalPromedio: isFinite(totalpromedio) ? totalpromedio.toLocaleString(undefined, { maximumFractionDigits: 0 }) : 0,
